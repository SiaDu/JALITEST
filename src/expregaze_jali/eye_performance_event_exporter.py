...
def generate_regulatory_blinks(
    ...
    generate_from_gaze: bool = False,
    generate_from_mask: bool = False,
    generate_from_heart: bool = False,
    generate_from_lid_state: bool = False,
    generate_long_gap_blinks: bool = True,
) -> list[dict[str, Any]]:
    """
    Rule-based blinks, generated after TextGrid timing resolve.

    Conservative default for v1:
      - only long no-blink gap subtle blinks are generated by default.
      - gaze/mask/heart/lid reset blinks are available, but disabled until
        performative blink/lid_state planning is stable.

    This avoids LLM performative blink fighting code-generated transition blink.
    """
    ...
    for event in resolved_events.get("events", []):
        ...
        if event_type == "gaze" and generate_from_gaze:
            candidates.append(
                (
        ...
        elif event_type == "mask" and generate_from_mask:
            candidates.append((float(event["resolved_time"]["start"]), "mask_change", str(event.get("id", ""))))
        elif event_type == "heart" and generate_from_heart:
            candidates.append((float(event["resolved_time"]["start"]), "heart_change", str(event.get("id", ""))))
        elif event_type == "lid_state" and generate_from_lid_state:
            candidates.append((float(event["resolved_time"]["start"]), "lid_state_change", str(event.get("id", ""))))
    ...
    if not generate_long_gap_blinks:
...
def export_eye_performance_events(
    ...
    regulatory_config_used = {
        "min_gap_frames": int(config.get("min_gap_frames", 45)),
        "long_gap_seconds": float(config.get("long_gap_seconds", 5.0)),
        "gaze_blink_offset_frames": int(config.get("gaze_blink_offset_frames", 1)),
        "generate_from_gaze": bool(config.get("generate_from_gaze", False)),
        "generate_from_mask": bool(config.get("generate_from_mask", False)),
        "generate_from_heart": bool(config.get("generate_from_heart", False)),
        "generate_from_lid_state": bool(config.get("generate_from_lid_state", False)),
        "generate_long_gap_blinks": bool(config.get("generate_long_gap_blinks", True)),
    }
    ...
    return {
        ...
        "diagnostics": {
            ...
            "regulatory_config": regulatory_config_used,
