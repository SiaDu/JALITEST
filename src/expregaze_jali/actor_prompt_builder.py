...
CAPABILITY_PROFILES: dict[str, dict[str, Any]] = {
    "mvp": {
        ...
        "tag_budget": {
            "gaze": "8-12 tags for a 30-45 second clip; maximum 14",
            "mask": "4-7 tags for a 30-45 second clip; maximum 9",
            "heart": "0-2 tags; only when internal state differs from visible mask",
        },
        "note": (
    ...
    "full_actor": {
        ...
        "tag_budget": {
            ...
            "mask": "4-7 tags for a 30-45 second clip; maximum 9",
            "heart": "0-2 tags; only when internal state differs from visible mask",
            "lid_state": "1 baseline + 2-3 major changes",
            "performative_blink": "0-2 tags; only when the blink itself is acting",
            "blink_suppression": "0-2 intervals; only when not blinking is the acting choice",
        },
        "note": (
...
def load_extra_config_texts(
    *,
    ...
    """
    Load small config files as raw text for prompt injection.

    Keeping these as text avoids adding a PyYAML runtime dependency just for prompt building.
    """
    return {
...
def build_actor_annotation_prompt(
    *,
    ...
    """Inject context, transcript, capability profile, and config into the prompt template."""
    ...
    replacements = {
        "{{context_pack}}": json.dumps(context_pack, ensure_ascii=False, indent=2),
        "{{capability_profile}}": json.dumps(capability_profile, ensure_ascii=False, indent=2),
